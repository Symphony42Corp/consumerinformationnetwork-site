...
## /auto-insurance serves the call page (Sept 21, 2026)
- Why: `/auto-insurance` is the Google phone-number verification URL and final URL of the two paused legacy call-only ads (756865355183, 762742866953; 888-802-1722). Since the Sept 2 rebuild it was a meta-refresh stub to the reviewed page, which shows 888-610-5950 — a Google re-check would find no 888-802-1722 at that address. Call-only ads cannot be recreated (creation removed Feb 2026). Source: Governor room, "Requirements: Phone-Number Verification Page Fix (/auto-insurance)", Sept 21 2026; approved by SPF ("Deploy everything per your plan"). SPF waived gate G1 (OpenAI Ads destination check).
- Change: removed the `/auto-insurance` entry from `redirects`; added `/auto-insurance` to the sitemap exclusion; added `src/pages/auto-insurance.astro` (same `CallLayout`, same `aiinCall` data as `/auto-insurance/call`). No new copy, no new content, no auto-dial.
- Accepted side effect: an old link to `/auto-insurance` now shows the call page instead of forwarding to the information page. The other five legacy forwards are unchanged.
- Known property: the page's canonical is `/auto-insurance/call` (inherent to byte-identity). Both addresses are noindex and both show the number.
- Verified locally: 13 pages built; exactly one file in `dist/` changed — `auto-insurance.html`, now byte-identical to `auto-insurance/call.html` (sha256 d2beab6f…6e1, 15,385 B); number 5× as text, `tel:+18888021722` 2×, noindex, tag AW-11394874943 with both labels, zero refresh tags. Reviewed page, call page, sitemap and the other five forwards unchanged. `.replit` regeneration: no diff (the `/auto-insurance` rewrite already existed). check-forbidden 18 files, 0 violations.

## Live verification (Sept 21, 2026, after publish of 9a47a64 at 23:22 GMT via the connector's publish_app — no manual click)
- `/auto-insurance` and `/auto-insurance/` on www and apex: HTTP 200, no redirect of any kind, zero refresh tags, 888-802-1722 in text, `tel:+18888021722` ×2, noindex, tag AW-11394874943 with both labels; bytes equal to the live `/auto-insurance/call` (15,503 B incl. Replit's injected script). Same result when fetched as AdsBot-Google-Mobile.
- Unchanged before → after (www and apex): `/auto-insurance/call`, `/auto-insurance-information-network` (8,634 B, 3e9a09b49f5e45b7), `sitemap-0.xml` (4b9257eaa2252934), and the five other legacy forwards. All 21 live pages/files equal the 9a47a64 build apart from Replit's script.
- Lighthouse mobile (simulated throttling) from this container was noisy on both byte-identical addresses: `/auto-insurance` 1.0–5.5 s LCP over six runs (best 1.0 s), `/auto-insurance/call` 1.1–5.2 s over three. Observed request times were all under 300 ms; slow runs coincided with high observed-RTT estimates. Parity with the call page holds; the absolute ≤ 1.5 s reading should be re-taken from a steadier environment.
